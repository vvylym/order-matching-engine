...
use super::sink::CollectingEventSink;
...
mod harness_api {
    ...
    #[test]
    fn incremental_sequence_starts_at_custom_base() {
        let mut s = IncrementalSequence::new(100);
        assert_eq!(s.next().unwrap(), 100);
    }

    #[test]
    fn shared_state_engine_wires() {
        let (_e, _sink, _book, _store) = engine_with_shared_state();
    }

    #[test]
    fn self_trade_rejection_engine_type() {
        let (engine, _sink) = engine_with_self_trade_rejection();
        let _typed: EngineWithSelfTradeRejection = engine;
    }

    #[test]
    fn all_engine_factories_construct() {
        let _ = engine_with_memory();
        let _ = engine_with_btree_book();
        let _ = engine_with_pool_level_book();
        let _ = engine_with_dash_skip_book();
        let _ = engine_with_book_noop::<InMemoryPriceBook>();
        let _ = add_cmd(
            1,
            1,
            Side::Buy,
